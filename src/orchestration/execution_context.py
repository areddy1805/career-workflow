...
class PipelineExecutionContext:
    ...
    def _extract_top_5(self, jobs: List[Any]) -> List[dict]:
        ...
        for j in jobs[:5]:
            ...
            top.append({"id": payload["pipeline_job_id"], "title": payload["title"], "company": payload["company"]})
        return top

    def start_stage(self, stage: str, input_jobs: List[Any]):
        if self.current_stage:
            raise RuntimeError(f"Cannot enter {stage} while {self.current_stage} is active.")
    ...
    def record_cache_hit(self):
        event = self.event_factory.create(self.current_stage or "Unknown", "CacheHit", {})
        self.bus.publish(event)
        
    def record_cache_miss(self):
        event = self.event_factory.create(self.current_stage or "Unknown", "CacheMiss", {})
        self.bus.publish(event)

    def emit_job_event(self, job: Any, event_type: str, extra: dict = None):
        payload, jid = self.registry.get_metadata(job, extra)
        event = self.event_factory.create(self.current_stage or "Unknown", event_type, payload, jid)
        self.bus.publish(event)
    ...
    def apply(self, job: Any, outcome: str, explanation: dict = None):
        self.emit_job_event(job, "JobApplied", {"outcome": outcome, "explanation": explanation or {}})
        self.complete(job)
    ...
    def skip(self, job: Any, reason: str, code: str = None):
        self.emit_job_event(job, "JobSkipped", {"reason": reason, "code": code or "SKIPPED"})
