...
def shell(active: str):
    ...
    with ui.header().classes("cw-header items-center px-5 gap-3"):
        ui.button(icon="menu", on_click=lambda: drawer.toggle()).props(
            'flat round color=grey-5 aria-label="Toggle navigation"'
        )
        ui.html('<div class="cw-brand">Career Workflow</div>')
        ui.space()
        search = (
            ui.input(placeholder="Search or jump…")
            .props("dense borderless prepend-icon=search")
            .classes("cw-top-search w-64 max-sm:hidden")
        )
    ...
    with ui.left_drawer(value=True).classes("cw-drawer") as drawer:
        with ui.row().classes("items-center gap-3 px-4 pt-5 pb-4"):
            ui.html(
                '<div style="width:40px;height:40px;border-radius:12px;display:grid;place-items:center;background:linear-gradient(145deg,#7368f4,#45cdec);font-weight:850;box-shadow:0 10px 28px rgba(100,110,255,.22)">CW</div>'
            )
            ui.html(
                '<div><div class="cw-brand">Career Workflow</div><div class="cw-brand-sub">OPERATIONS CONTROL PLANE</div></div>'
            )
