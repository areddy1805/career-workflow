...
def page():
    ...
    with ui.column().classes("cw-content gap-5"):
        ...
        page_header(
            "OPERATE",
            "Pipeline Control",
            "Configure, execute, and observe the application engine while preserving process truth.",
            process,
        )
        hero(
            "Mission control",
            "Dry run is the default safety boundary. Live mode requires explicit confirmation. Process telemetry and terminal artifacts are displayed separately.",
            process,
        )
        ...
        with ui.element("div").classes("cw-split w-full"):
            with ui.column().classes("gap-3"):
                section(
                    ...
                    "Latest terminal artifact · historical truth",
                )
            ...
            with ui.card().classes("cw-card p-5 gap-4"):
                ...
                limit = ui.number(
                    "Application ceiling", value=500, min=1, max=1000
                ).classes("w-full")
                ...
                ui.button(
                    "Launch Pipeline", icon="rocket_launch", on_click=launch
                ).props("color=primary unelevated").classes("w-full")
                ui.button(
                    "Refresh State", icon="refresh", on_click=lambda: render_state()
        ...
        def render_state():
            ...
            with metrics:
                metric_card(
                    "Process", state.get("status", "IDLE"), "live launcher state"
                )
                metric_card("PID", state.get("pid") or "—", "process owner")
                metric_card(
                    "Mode",
            ...
            content = (
                read_pipeline_log()
                if running
                else "No active process. Historical output is available in Run Inspector."
            )
            log_box.set_content(
                f'<pre style="margin:0;white-space:pre-wrap">{html.escape(content)}</pre>'
            )

        section(
            "Live output",
            "Active launcher log · refreshed every two seconds while this page is open",
        )
