...
from career_ui.services.control_center import (
    read_manual_jobs, add_manual_job, update_manual_job_status, MANUAL_JOB_SOURCES,
    read_manual_action_queue, update_external_action_status,
)
...
def page():
    ...
    with ui.column().classes('cw-content gap-5'):
        page_header('WORK QUEUE', 'External Apply Queue', 'Pipeline-detected external applications and manually sourced opportunities in one operational workspace.')
        ...
        def refresh_auto():
            ...
            with auto_host:
                section('Pipeline-detected external applications', 'Jobs the pipeline cannot submit directly')
                ...
                if frame.empty:
                    ...
                    with ui.row().classes('items-end gap-3'):
                        ...
                        def move_external():
                            ...
                            refresh_auto()
                        ui.button('Update External Job', on_click=move_external).props('color=primary unelevated')

        def refresh_manual():
            manual_host.clear()
            frame = read_manual_jobs()
            with manual_host:
                section('Manually sourced opportunities', 'LinkedIn, referrals, company careers, and other sources')
                if frame.empty:
                    ...
                    with ui.row().classes('items-end gap-3'):
                        ...
                        def move_manual():
                            ...
                            refresh_manual()
                        ui.button('Update Manual Job', on_click=move_manual).props('color=primary unelevated')

        with ui.expansion('Add manually sourced opportunity', icon='add_circle').classes('cw-card w-full'):
            with ui.grid(columns=2).classes('w-full gap-3 p-3'):
                title=ui.input('Title'); company=ui.input('Company'); location=ui.input('Location')
                source=ui.select(list(MANUAL_JOB_SOURCES),label='Source',value='LinkedIn')
                url=ui.input('Source URL'); priority=ui.select(['P1','P2','P3'],label='Priority',value='P2')
                notes=ui.textarea('Notes').classes('col-span-2')
            def add():
                try:
                    add_manual_job(title=title.value or '', company=company.value or '', location=location.value or '', source=source.value, source_url=url.value or '', priority=priority.value, notes=notes.value or '')
                    ui.notify('Job added', type='positive'); refresh_manual()
                except Exception as exc:
                    ui.notify(str(exc), type='negative')
            ui.button('Add Job',on_click=add).props('color=primary unelevated').classes('m-3')

        refresh_auto(); refresh_manual()
