...
def page():
    ...
    with ui.column().classes("cw-content gap-5"):
        ...
        page_header(
            "OPERATIONS",
            "Command Center",
            "Live process truth, latest artifact throughput, conversion, and work requiring attention.",
            process,
        )
        hero(
            "Application engine overview",
            "Live launcher state and immutable run artifacts are deliberately separated. Operate from Pipeline Control; inspect outcomes here.",
            process,
        )
        with ui.element("div").classes("cw-grid-5 w-full"):
            ...
            metric_card(
                "Selected", f"{run_count(run,'selected'):,}", "qualified opportunities"
            )
            metric_card(
                "Portfolio", f"{int(apps.get('total',0)):,}", "lifetime applications"
            )
        if artifact == "ORPHANED" and not running:
            callout(
                "Artifact requires attention",
                "The latest run artifact is ORPHANED, but no live process owns it. Treat the artifact as historical state; use Run Inspector for evidence or Pipeline Control for a new execution.",
            )
        section(
            "Execution map",
            "Latest artifact-backed stage progression · not live process state",
        )
