...
def page():
    ...
    with ui.column().classes("w-full max-w-7xl mx-auto p-6 gap-6"):
        ...
        with metrics_grid(cols=7):
            metric_card("Apps Today", f"{run_count(run,'submitted'):,}", "Run submissions")
            metric_card("Total Apps", f"{int(apps.get('total',0)):,}", "Lifetime tracked")
            metric_card("Man. Queue", f"{manual_queue_count:,}", "Requires review")
            metric_card("WF Queue", f"{workflow_queue_count:,}", "Qualified")
