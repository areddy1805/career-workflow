...
def page():
    ...
    with ui.column().classes("cw-content gap-5"):
        page_header(
            "WORKSPACE",
            "Jobs",
            "Search, filter, and inspect the job portfolio captured by the workflow.",
        )
        frame = read_applications()
        if frame.empty:
            empty_state(
                "No job records available", "Run acquisition or import jobs first."
            )
            return
        search = (
            ui.input(placeholder="Search title, company, location…")
        ...
        def render():
            ...
            if q:
                cols = [
                    c
                    for c in ("title", "company", "location", "subtrack", "priority")
                    if c in f.columns
                ]
                mask = (
                    f[cols]
